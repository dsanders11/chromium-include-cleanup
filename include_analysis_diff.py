...
def get_archived_include_analysis(analysis_url: str) -> IncludeAnalysisOutput:
    ...
    return parse_raw_include_analysis_output(include_analysis_json)
...
def include_analysis_diff(
    ...
    for min_days_delta in (7, 30):
        # TODO - Determine this from the filenames alone instead of opening the files
        for previous_analysis in analysis_list:
            previous_analysis_date = datetime.fromisoformat(previous_analysis[0])
            delta = analysis_date - previous_analysis_date
            ...
            if delta.days >= min_days_delta:
                ...
                previous_analyses[delta.days] = parse_raw_include_analysis_output(
                    previous_include_analysis_file.read()
                )
    ...
    for previous_analysis in previous_analyses.values():
        for filename in filenames:
            ...
            if flag_node:
                yield (previous_analysis["revision"], filename, "", str(difference))
            ...
            for header in include_analysis["esizes"][filename]:
                ...
                if flag_edge:
                    ...
                    yield (previous_analysis["revision"], filename, header, str(difference))
...
if __name__ == "__main__":
    try:
        sys.exit(extract_include_analysis_list())
        # sys.exit(main())
